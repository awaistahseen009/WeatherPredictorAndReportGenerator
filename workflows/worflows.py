...
# Default config if load_config is not available
# Pydantic model for restaurant information
...
def generate_plot(state: WeatherState) -> WeatherState:
    ...
    plt.title(f'Hourly Temperature Forecast for {config["location"]["city"]}', fontsize=16, fontweight='bold')
...
def generate_html(state: WeatherState) -> WeatherState:
    report = state["report"]
...
def create_langgraph_workflow():
    ...
    workflow.add_node("analyze_forecast", analyze_forecast)
...
# Run the LangGraph workflow and return the HTML path
def run_langgraph_report(forecast_data: pd.DataFrame, historical_avg: Optional[float] = None) -> str:
    workflow = create_langgraph_workflow()
    result = workflow.invoke({"forecast_data": forecast_data, "historical_avg": historical_avg})
    return result
